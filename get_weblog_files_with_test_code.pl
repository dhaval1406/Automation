...
sub extract_logs_and_execute{
	...
	foreach my $log_file (@log_files){
		...
		# Join file names to pass via commandline arguments to R scripts		
		$filenames_args .= ' '.$log_file_txt;			 
	}
		
	# Calling R scripts with command line arguments
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Data_Load_Prod.R > $logs_dir\\Data_Load_Prod_$end_date.txt") == 0 or die "I can't take it anymore. Died for Data_Load_Prod.R";
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Analysis1.R > $logs_dir\\Analysis1_$end_date.txt") == 0 or die "I can't take it anymore. Died for Analysis1.R";
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Analysis2.R > $logs_dir\\Analysis2_$end_date.txt") == 0 or die "I can't take it anymore. Died for Analysis2.R";
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Analysis3.R > $logs_dir\\Analysis3_$end_date.txt") == 0 or die "I can't take it anymore. Died for Analysis3.R";
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Analysis4.R > $logs_dir\\Analysis4_$end_date.txt") == 0 or die "I can't take it anymore. Died for Analysis4.R";
	system("RTerm --vanilla --args $filenames_args < $r_script_dir/Analysis5.R > $logs_dir\\Analysis5_$end_date.txt") == 0 or die "I can't take it anymore. Died for Analysis5.R";
}
...
sub cleanup_and_archive_log_file{
	...
	# Archive log files
	system ("C:/\"Program Files\"/7-Zip/7z.exe  a -y $archive_destination $file_storage_dir\\*"); 
...
sub usage{    
    logger("Synopsis:");
    logger("     ./find_overrides.pl -t <weblog_file_type>");
    logger("\nDescription:");
...
#==================================== TEST area ==============================
	# #my $content = get($web_address) or die 'Unable to get page';
	# my $req = GET $search_log_address ;
	# $req->authorization_basic('29_ykaneda', '6T06YeBu0F5si1s');
	# my $res = $ua->request($req, "search_log_file.txt");
	
	# ($res->is_success) ? print "ok\n" : print $res->status_line, "\n";
		
	#print Dumper keys %{$res};<STDIN>;
	# print Dumper $res->{'_previous'}; <STDIN>;
	# print Dumper $res->{'_msg'}; <STDIN>;
	# print Dumper $res->{'_content'}; <STDIN>;
	# print Dumper $res->{'_protocol'}; <STDIN>;
	# print Dumper $res->{'_headers'}; <STDIN>;
	# print Dumper $res->{'_rc'}; <STDIN>;
	# print Dumper $res->{'_request'}; <STDIN>;

	# print Dumper $req;<STDIN>;
	# #print Dumper $content; <STDIN>;
...
# use Data::Dumper;  
 ...
 # HTTP::Request->new(
  ...
  # )
# print "I am going to print value for ua handle \n";
# print Dumper $ua; <STDIN>;
  
# #  $ua->request(POST 'http://somewhere/foo', [foo => bar, bar => foo]);

#my $web_address = 'http://172.24.210.21/reference/download/download.php?startdate[Y]=2013&startdate[m]=06&startdate[d]=11&endtdate[Y]=2013&endtdate[m]=06&endtdate[d]=14&table=worldmvlog.us_dt_search_fix_ip';